...
class BEVDataset:
    def __init__(self, label, label_file_end, traj_config, data_config):
        ...
        if BEVDataset.cached_input_data is None:
            data_gt, detection_dict_gt = get_input_data(cache=data_config['use_input_cache'],
                                                cache_path=data_config['input_cache_path'])
            BEVDataset.cached_input_data = (data_gt, detection_dict_gt)
        else:
            data_gt, detection_dict_gt = BEVDataset.cached_input_data
        ...
        input_files = filtered_glob(os.path.join(self.splits_path, self.dataset_fold, label, '*.txt'))
        meta_df = self._balance_locs(input_files, self.balance)

        for input_file in tqdm(input_files, f'Processing {label} input...', dynamic_ncols=True):
            ...
            full_name = input_file.split('/')[-1].replace(label_file_end, '')
            ...
            def process_scene(frame_start):
                ...
                def process_agent(agent_id, agent_df):
                    ...
                    ego_obs = np.concatenate([ego_track[:self.hist_len], np.ones((self.hist_len,1), int), \
                                              np.ones((self.hist_len, 1), int)*agent_id], axis=-1)
                    # Make it the whole thing...for sgnet and such
                    ego_pred = np.concatenate([ego_track[:], np.ones((self.fut_len + self.hist_len,1),int), \
                                               np.ones((self.fut_len + self.hist_len,1), int)*agent_id], axis=-1)

                    ego_start_frame = file_data[file_data.agent_id == agent_id].frame_id.min()
                    resnet_offset = (frame_start - ego_start_frame)//self.frame_skip

                    img_folder = os.path.join(self.imgs_path, full_name, f'agent{agent_id}')
                    img_files = [os.path.join(img_folder, f'idx{resnet_offset + i}.jpg') for i in range(self.hist_len)]
                    assert all([os.path.exists(x) for x in img_files]), 'Missing image file; is frame_skip correct?'
                    imgs = np.array(img_files)
                    
                    return ego_obs, ego_pred, imgs

                tmp_obs = []
                tmp_pred = []
                tmp_imgs = []
                for agent_id, agent_df in scene_data.groupby('agent_id'):
                    ego_obs, ego_pred, imgs = process_agent(agent_id, agent_df)
                    tmp_obs.append(ego_obs)
                    tmp_pred.append(ego_pred)
                    tmp_imgs.append(imgs)
                return n_agents, np.stack(tmp_obs), np.stack(tmp_pred), np.stack(tmp_imgs)

            frame_starts = list(range(min_frame, max_frame + 1, self.step_skip*self.frame_skip))
            results = t_map(process_scene, frame_starts, desc = f'Processing scenes in {input_file.split("/")[-1]}', dynamic_ncols=True, leave=False)
            for result in tqdm(results, desc=f'Processing scenes more fully...', dynamic_ncols=True, leave=False):
                n_agents, tmp_obs, tmp_pred, tmp_imgs = result
                if n_agents is None:
                    continue
                
                for idx in range(n_agents):
                    new_obs, new_pred = self._get_detections(tmp_obs, tmp_pred, idx)
                    cur_scene_id += 1
                    # Now we need to make it so that each person becomes their own ego
                    all_obs = concat_init(all_obs, new_obs)
                    all_pred = concat_init(all_pred, new_pred)
                    # Overwrite img_paths to be ego only...
                    # Apply noise here if needed
                    scene_ids.append(cur_scene_id)
                    seq_len_obs.append(len(new_obs))
                    seq_len_pred.append(len(new_pred))
                all_img_paths = concat_init(all_img_paths, tmp_imgs)
    ...
    def _get_detections(self, tmp_obs, tmp_pred, idx: int):
        assert len(tmp_obs), 'At least one required'
        if len(tmp_obs) == 1:
            return tmp_obs, tmp_pred
        # Potentially apply noise here
        ego_obs, ego_pred = tmp_obs[idx][np.newaxis, ...], tmp_pred[idx][np.newaxis, ...]
        det_idxs = np.concatenate([np.arange(0, idx), np.arange(idx + 1, len(tmp_obs))])
        det_obs, det_pred = tmp_obs[det_idxs], tmp_pred[det_idxs]
        def obs_to_df(obs):
            obs = obs.reshape(-1, obs.shape[-1])
            df = pd.DataFrame(obs, columns=['x', 'y', 'yaw', 'img_x', 'img_y', 'valid', 'det_id'])
            df['orig_id'] = df['det_id']
            return df
            
        if self.noise is not None:
            dets_df = obs_to_df(det_obs)
            ...
            if len(dets_df):
                ...
                next_agent_id = None
            n_obs = ego_obs.shape[1]
            obs_frames = np.arange(n_obs)
            dets_df['frame_id'] = 0
            for assigned_id, noise_agent_df in dets_df.groupby('det_id'):
                dets_df.loc[dets_df.det_id == assigned_id, 'frame_id'] = obs_frames
                ...
                for row_idx in noise_agent_df.index:
                    ...
                    noise_x = self.noise['mu'] + self.noise['std']*self.noise_random.randn() + row.x
                    noise_z = self.noise['mu'] + self.noise['std']*self.noise_random.randn() + row.y
                    dets_df.loc[row_idx, ['det_id', 'x', 'y']] = [cur_id, noise_x, noise_z] 
            ...
            for det_agent, det_agent_data in dets_df.groupby('det_id'):
                det_obs_dict[f'{det_agent}_orientation'] = [np.nan]*n_obs
                det_obs_dict[f'{det_agent}_val'] = [False]*n_obs
                det_obs_dict[f'{det_agent}_x'] = [np.nan]*n_obs
                det_obs_dict[f'{det_agent}_y'] = [np.nan]*n_obs
                det_obs_dict[f'{det_agent}_img_x'] = [np.nan]*n_obs
                det_obs_dict[f'{det_agent}_img_y'] = [np.nan]*n_obs
                assert det_agent_data.orig_id.nunique() == 1, 'Unexpected number of original id'
                orig_id = det_agent_data.orig_id.unique()[0]
                det_obs_dict[f'{det_agent}_orig_id'] = [orig_id]*n_obs
            ...
            for det_agent, det_agent_data in dets_df.groupby('det_id'):
                ...
                det_obs_df.loc[det_agent_data.frame_id, [f'{det_agent}_x', f'{det_agent}_y', f'{det_agent}_orientation']] = \
                    det_agent_data[['x', 'y', 'yaw']].values
                det_img_x = det_agent_data['img_x'] + 0.5*det_agent_data['img_x']
                det_img_y = det_agent_data['img_y'] + 0.5*det_agent_data['img_y']
            ...
            if self.hungarian_match:
                ...
                # Filter out nan from det_pred's future period...decide later
                det_pred_filt = [x for x in det_pred if not np.isnan(x[:self.hist_len, :2].astype(np.float32)).any()]
                if not len(det_pred_filt) or not len(det_obs):
                    return ego_obs, ego_pred
            ...
            if not self.hungarian_match and self.alignment and self.noise is not None:
                ...
                for idx in range(len(det_obs)):
                    ...
                    gt_ids.append(idx)
                det_pred = new_det_pred

        return np.concatenate([ego_obs, det_obs]), np.concatenate([ego_pred, det_pred])
...
BEVDataset.cached_input_data = None

    
def bev_seq_collate(data):
    (
        obs_list,
        pred_list,
        obs_resnet_list,
...
def bev_dataset_name(data_config, traj_config, out_name):
    if 'balance' in traj_config and traj_config['balance']:
