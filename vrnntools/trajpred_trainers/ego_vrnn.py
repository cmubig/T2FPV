...
from vrnntools.trajpred_models.tp_ego_vrnn import TrajPredEgoVRNN
...
class EgoVRNNTrainer(BaseTrainer):
    ...
    def train_epoch(self, epoch: int, train_corr=False) -> dict:
        ...
        for i, batch in tqdm(enumerate(self.train_data), dynamic_ncols=True, desc=epoch_str, total=self.num_iter):
            ...
            if self.pretrained_pred is None:
                ...
                kld, nll, _ = self.model(hist_rel[:hist_len], hist_resnet, hist_abs[:hist_len], hist_seq_start_end)
            else:
                with torch.no_grad():
                    kld, nll, _ = self.model(hist_rel[:hist_len], hist_resnet, hist_abs[:hist_len], hist_seq_start_end)
        ...
        for i, batch in tqdm(enumerate(self.val_data), dynamic_ncols=True, desc=epoch_str, total=self.eval_num_iter):
            ...
            with torch.no_grad():
                ...
                if self.pretrained_pred is not None:
                    _, _, h = self.pretrained_pred(hist_rel, hist_resnet, hist_abs, hist_seq_start_end)
                else:
                    _, _, h = self.model(hist_rel, hist_resnet, hist_abs, hist_seq_start_end)
    ...
    def setup(self) -> None:
        ...
        self.model = TrajPredEgoVRNN(model_info, self.device).to(self.device)
