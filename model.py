...
class NAOMI(nn.Module):
    ...
    def forward(self, data, ground_truth):
        ...
        sample = data.clone().to(data.device)[..., 1:]
            ...
            for l, dim in self.dims.items():
                ...
                if t + 2 * step_size <= data.shape[0]:
                    ...
                    if self.stochastic:
                        ...
                        sample[t+step_size] = dec_mean_t


        return loss / count / data.shape[1], sample

    def sample(self, data_list):
        # data_list: seq_length * (1 * batch * 11)
        ret = []
        seq_len = len(data_list)
        h = Variable(torch.zeros(self.params['n_layers'], data_list[0].shape[1], self.rnn_dim))
        if self.params['cuda']:
            h = h.cuda()
        
        h_back_dict = {}
        h_back = Variable(torch.zeros(self.params['n_layers'], data_list[0].shape[1], self.rnn_dim))
        if self.params['cuda']:
            h_back = h_back.cuda()  
...
class SingleRes(nn.Module):
    ...
    def sample(self, data_list):
        # data_list: seq_length * (1 * batch * 11)
        h = Variable(torch.zeros(self.n_layers, data_list[0].shape[1], self.rnn_dim))
        h_back = Variable(torch.zeros(self.n_layers, data_list[0].shape[1], self.rnn_dim))
        if self.params['cuda']:
            h, h_back = h.cuda(), h_back.cuda()
